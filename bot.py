...
TOKEN = "8687509526:AAGipVLVR5ICeRaQDbnZ-qsZqRhPrHhiThA" 

# ── Join requirements ─────────────────────────────────────────────────────────
join_channel_id = -1003667013674       # Replace with your channel ID
join_chat_id    = -1004323566402       # Replace with your group ID

CHANNEL_LINK = "https://t.me/cyberworldXo"
GROUP_LINK   = "https://t.me/+uY4WaWnJ7GQxYWE0"
...
async def cmd_sh(message: types.Message):
    ...
    # ── Get random site (API does not require sites) ──────────────────────────
    site = "-"
...
_chk = asyncio.create_task(checker_bridge.check_card_site(cc_str, site, proxy_data, proxy_list))
...
async def _msh_check_single(
    ...
    site = random.choice(sites_list) if sites_list else "-"
    ...
    async with sem:
        try:
            proxy_list = get_user_proxies(user_id)
proxy_list = get_user_proxies(user_id)
result = await checker_bridge.check_card_site(cc_str, site, proxy_data, proxy_list)
...
async def _filter_check_site(site: str, proxy_data: dict | None) -> dict:
    ...
    try:
        proxy_list = get_user_proxies(user_id)
result = await checker_bridge.check_card_site(_FILTER_CC, site, proxy_data, proxy_list)
    except Exception as e:
